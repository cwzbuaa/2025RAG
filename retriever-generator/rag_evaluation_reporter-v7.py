...
API_KEY = ""  # 必须与agent_backend.py中的API_KEY一致
...
def calculate_ragas_metrics(results: list, is_rag: bool = True):
    ...
    for r in valid_results:
        ...
        # 构建数据点 - 确保所有字段都是正确的类型
        data_point = {
            "question": str(r["question"]),  # 确保是字符串
            "answer": str(r["answer"]),      # 确保是字符串
            "contexts": contexts,            # 字符串列表
            "ground_truth": ground_truth if ground_truth else "无标准答案"  # 字符串
        }
